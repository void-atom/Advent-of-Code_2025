...
func part_one_logic(input string, dial_value int) int {
	zero_count := 0
	for i := 0; i < len(input); {
		if i >= len(input) {
			break
...
func part_two_logic(input string, dial_value int) int {
	zero_count := 0
	for i := 0; i < len(input); {
...
func main() {
	...
	fmt.Printf("Part one answer = %d\n", part_one_logic(input, dial))
	fmt.Printf("Part two answer = %d\n", part_two_logic(input, dial))
